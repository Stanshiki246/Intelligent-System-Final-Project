...
#Alpha Beta Pruning and Q-Learning Checkers
...
def play_n_games(player1, player2, num_games, move_limit):
    ...
    print('Statistics of ',player1.name,' vs ',player2.name,' in ',num_games,' rounds:')
    player1_win=0
    player2_win=0
    tied=0
    for j in range(num_games):
        ...
        while not game_board.is_game_over() and move_counter < move_limit:
            ...
            print('Result of Board in round ',j+1,':')
            game_board.print_board()
            game_board.reset_board()

    print('Number of tied: ',tied)
    print('Number of Player 1 wins: ',player1_win)
    print('Number of Player 2 Wins: ',player2_win)
...
def switch_board_players(board):
    ...
    for row_index in range(len(board.spots)):
        for col_index in range(len(board.spots[row_index])):
...
def player_vs_ai(player2):
    game_board = Board()
    player2.set_board(game_board)
    outcome_counter=[-1,-1,-1,-1,-1,-1]
    step=0
    surrender=False
    while not game_board.is_game_over() and not surrender:
        game_board.print_board()
        player_input=[]
        start_input=input('Select Movement for example X1,Y1: ')
        if start_input == 'q':
            surrender=True
        else:
            temp=start_input.split(',')
            temp2=[]
            for i in temp:
                temp2.append(int(i))
            player_input.append(temp2)
            end_input=input('Select Movement for example X2,Y2: ')
            temp=end_input.split(',')
            temp3=[]
            for i in temp:
                temp3.append(int(i))
            player_input.append(temp3)
            game_board.make_move(player_input)
            game_board.print_board()
            player_input.clear()
            temp2.clear()
            temp3.clear()
            start = time()
            step+=1
            game_board.make_move(player2.get_next_move())
            calculation=time() - start
            print('Total Steps: ',step)
            print('Running time: ',calculation,'s')
    else:
        piece_counter = get_number_of_pieces_and_kings(game_board.spots)
        if piece_counter[0] != 0 or piece_counter[2] != 0:
            if piece_counter[1] != 0 or piece_counter[3] != 0:
            ...
            outcome_counter[0]=1
        outcome_counter[1] = 0
        outcome_counter[2] = piece_counter[0]
        outcome_counter[3] = piece_counter[1]
        outcome_counter[4] = piece_counter[2]
        outcome_counter[5] = piece_counter[3]

        #player2.game_completed()
        print('Game Over')
        game_board.reset_board()

#Q-Learning Player Agent
LEARNING_RATE = .01
DISCOUNT_FACTOR = .1
TRAINING_RANDOM_MOVE_PROBABILITY = .5
#Alpha Beta Player Agent
ALPHA_BETA_DEPTH = 3
#How many wins?
...
#Set AI Agents
Player1=Q_Learning_AI(LEARNING_RATE,DISCOUNT_FACTOR,TRAINING_RANDOM_MOVE_PROBABILITY)
...
while True:
    print('1. Player vs AlphaBeta')
    print('2. Player vs Q-Learning')
    print('3. Exit')
    choice=int(input('Choose: '))
    if choice == 1:
        player_vs_ai(Player2)
    elif choice == 2:
        player_vs_ai(Player1)
    elif choice == 3:
        print('See You')
        break
    else:
